...
kernel void main0(constant int& uLoadAction [[buffer(2)]], constant int2& uFramebufferTileSize [[buffer(5)]], constant int2& uTextureMetadataSize [[buffer(7)]], constant int& uCtrl [[buffer(13)]], constant float2& uFramebufferSize [[buffer(0)]], constant float2& uTileSize [[buffer(1)]], constant float4& uClearColor [[buffer(3)]], constant float2& uColorTextureSize0 [[buffer(8)]], constant float2& uMaskTextureSize0 [[buffer(9)]], constant float4& uFilterParams0 [[buffer(10)]], constant float4& uFilterParams1 [[buffer(11)]], constant float4& uFilterParams2 [[buffer(12)]], const device bFirstTileMap& _1502 [[buffer(4)]], const device bTiles& _1547 [[buffer(6)]], texture2d<float, access::read_write> uDestImage [[texture(0)]], texture2d<float> uTextureMetadata [[texture(1)]], texture2d<float> uColorTexture0 [[texture(2)]], texture2d<float> uMaskTexture0 [[texture(3)]], texture2d<float> uDestTexture [[texture(4)]], texture2d<float> uGammaLUT [[texture(5)]], sampler uTextureMetadataSmplr [[sampler(0)]], sampler uColorTexture0Smplr [[sampler(1)]], sampler uMaskTexture0Smplr [[sampler(2)]], sampler uDestTextureSmplr [[sampler(3)]], sampler uGammaLUTSmplr [[sampler(4)]], uint3 gl_WorkGroupID [[threadgroup_position_in_grid]], uint3 gl_LocalInvocationID [[thread_position_in_threadgroup]])
{
    ...
    while (tileIndex >= 0)
    {
        for (int subY_1 = 0; subY_1 < 4; subY_1++)
        {
            ...
            int alphaTileIndex = int(_1547.iTiles[(tileIndex * 4) + 2] << uint(8)) >> 8;
            uint tileControlWord = _1547.iTiles[(tileIndex * 4) + 3];
        ...
        tileIndex = int(_1547.iTiles[(tileIndex * 4) + 0]);
    }
