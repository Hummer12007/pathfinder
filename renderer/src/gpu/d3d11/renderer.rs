...
use pathfinder_gpu::{BufferTarget, ComputeDimensions, ComputeState, Device, ImageAccess, RenderTarget, UniformData};
...
pub(crate) struct RendererD3D11<D> where D: Device {
    ...
    vertex_arrays: VertexArraysD3D11<D>,
...
impl<D> RendererD3D11<D> where D: Device {
    pub(crate) fn new(core: &mut RendererCore<D>, resources: &dyn ResourceLoader)
        ...
        let quad_vertex_positions_buffer =
            core.allocator.get_buffer(core.quad_vertex_positions_buffer_id);
        let quad_vertex_indices_buffer =
            core.allocator.get_buffer(core.quad_vertex_indices_buffer_id);
        let programs = ProgramsD3D11::new(&core.device, resources);
        let vertex_arrays = VertexArraysD3D11::new(&core.device,
                                                   &programs,
        ...
        RendererD3D11 {
            ...
            scene_buffers: SceneBuffers::new(&mut core.allocator, &core.device),
    ...
    pub(crate) fn upload_scene_d3d11(&mut self,
                                     core: &mut RendererCore<D>,
    ...
    fn allocate_tiles_d3d11(&mut self, core: &mut RendererCore<D>, tile_count: u32) -> BufferID {
    ...
    pub(crate) fn prepare_tiles_d3d11(&mut self,
        ...
        let first_tile_map_buffer_id = self.allocate_first_tile_map_d3d11(core);

        let propagate_metadata_buffer_ids =
            self.upload_propagate_metadata_d3d11(core,
        ...
        for _ in 0..2 {
            microlines_storage = self.dice_segments_d3d11(core,
        ...
        for _ in 0..2 {
            self.bound_d3d11(core,
                             tiles_d3d11_buffer_id,
                             batch.tile_count,
                             &batch.prepare_info.tile_path_info);

            self.upload_initial_backdrops_d3d11(core,
                                                propagate_metadata_buffer_ids.backdrops,
                                                &batch.prepare_info.backdrops);

            fill_buffer_info = self.bin_segments_d3d11(core,
                                                       &microlines_storage,
        ...
        let alpha_tiles_buffer_id = self.allocate_alpha_tile_info_d3d11(core, batch.tile_count);

        let propagate_tiles_info =
            self.propagate_tiles_d3d11(core,
    ...
    fn allocate_first_tile_map_d3d11(&mut self, core: &mut RendererCore<D>) -> BufferID {
        core.allocator.allocate_buffer::<FirstTileD3D11>(&core.device,
                                                         core.tile_size().area() as u64,
                                                         BufferTag::FirstTileD3D11)
    }

    fn allocate_alpha_tile_info_d3d11(&mut self, core: &mut RendererCore<D>, index_count: u32)
                                      -> BufferID {
    ...
    pub(crate) fn draw_tiles_d3d11(&mut self,
                                   core: &mut RendererCore<D>,
                                   tiles_d3d11_buffer_id: BufferID,
                                   first_tile_map_buffer_id: BufferID,
...
impl SceneBuffers {
    fn new<D>(allocator: &mut GPUMemoryAllocator<D>, device: &D) -> SceneBuffers where D: Device {
        SceneBuffers {
            draw: SceneSourceBuffers::new(allocator, device),
            clip: SceneSourceBuffers::new(allocator, device),
        }
    }
...
impl SceneSourceBuffers {
    fn new<D>(allocator: &mut GPUMemoryAllocator<D>, device: &D) -> SceneSourceBuffers
              where D: Device {
        SceneSourceBuffers {
            points_buffer: None,
            points_capacity: 0,
